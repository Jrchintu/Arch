...
chrooot() {
	curl -L https://github.com/Jrchintu/CDN/raw/main/ARCH/chroot.sh -o /mnt/chroot.sh
...
de() {
	...
	if [[ "$DESKTOPENV" = '1' ]]; then
		...
		pacstrap /mnt xfce4 xorg-server lightdm lightdm-gtk-greeter gtk-engine-murrine gtk-engines xfce4-screenshooter \
			xfce4-whiskermenu-plugin xfce4-pulseaudio-plugin xfce4-battery-plugin network-manager-applet xdg-desktop-portal \
			gvfs gvfs-mtp mtpfs thunar-media-tags-plugin thunar-archive-plugin accountsservice
		arch-chroot /mnt bash -c "systemctl enable --now lightdm && exit"
...
main() {
	...
	array=(updates partition mounter base chrooot de)
	stepno=$((stepno - 1))
...
umount -fvR /mnt && wait
umount -fvR /mnt/home && wait
umount -fvR /mnt/boot/efi && wait
swapoff -av && wait
